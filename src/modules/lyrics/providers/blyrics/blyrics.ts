...
export default async function bLyrics(
	...
	if (metadata) {
		...
		let transliterations = metadata.iTunesMetadata!.find(
			(e) => e.transliterations,
		);
		...
		if (
			...
			transliterations.transliterations.length > 0
		) {
			transliterations.transliterations[0].transliteration.forEach(
				(transliteration, i) => {
					...
					if (line && line.startsWith("L")) {
						...
						if (lineIndex < lyrics.length) {
							...
							let parseResult = parseLyricPart(
	...
	if (isWordSynced) {
		providerParameters.sourceMap["bLyrics-richsynced"].lyricSourceResult =
			result;
		providerParameters.sourceMap["bLyrics-synced"].lyricSourceResult = null;
	} else {
		providerParameters.sourceMap["bLyrics-richsynced"].lyricSourceResult = null;
